...
def get_workspace_status(customer_id):
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            return data.get("account", {}).get("status", "UNKNOWN").upper()
        else:
            return "UNKNOWN"
...
total_status = len(all_customers)
current_status = 0
print("\nFetching workspace status...\n")
...
for c in all_customers:
    ...
    # Fetch status
    status = get_workspace_status(customer_id)
    ...
    print(f"Status fetched: {current_status}/{total_status}", end="\r")
...
for msp_id, msp_info in msp_dict.items():
    ...
    color = get_status_color(msp_info["status"])

    tenant_table = ""

    if tenants:
        tenant_table += f'<div id="{unique_id}" class="tenant-wrapper">'
        tenant_table += """
        <table style="margin-top:6px;width:95%;font-size:13px;">
        <tr><th>Tenant</th><th>ID</th></tr>
        """

        for t in tenants:
            tenant_color = get_status_color(t["status"])
            tenant_table += f"""
            <tr>
                <td>{t['name']} <span class="status-badge" style="background:{tenant_color};">{t['status']}</span></td>
                <td>{t['customer_id']}</td>
            </tr>
            """

        tenant_table += "</table></div>"

    msp_block = f"""
        <button class="toggle-btn" onclick="toggleTenants('{unique_id}')">
            <span id="arrow-{unique_id}">▶</span>
            {msp_info['name']}
            <span class="status-badge" style="background:{color};">{msp_info['status']}</span>
        </button>
        {tenant_table}
    """
...
for i in range(max_rows):
    ...
    if i < len(standalone):
        sa_name, sa_id, sa_status = standalone[i]
        sa_color = get_status_color(sa_status)
        sa_name = f'{sa_name} <span class="status-badge" style="background:{sa_color};">{sa_status}</span>'
    else:
        sa_name, sa_id = "", ""
